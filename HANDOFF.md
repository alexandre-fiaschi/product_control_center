...
- **TOC regeneration — Word driven via AppleScript at render time (Unit 11).** `render_release_notes()` calls `regenerate_fields(docx_path)` right after `doc.save()`, which shells `/usr/bin/osascript` against [_regen_fields.applescript](backend/app/pipelines/docs/_regen_fields.applescript) to open the DOCX in Word, iterate every `table of contents of document 1`, call `update` on each, save, close. Adds ~13s per call. Real page numbers, no XML surgery. Word has a full native AppleScript dictionary; LibreOffice's UNO path is blocked on modern macOS by Apple Launch Constraints on `LibreOfficePython` (SIGKILL on launch from any non-`soffice` parent). `mark_toc_dirty()` still runs as a defensive backstop so Word's on-open regen works even for DOCXs that somehow miss the osascript step.
  - **Sandbox gotcha:** Word is sandboxed — on the first render, macOS prompts "Grant File Access" for the patches folder. Click Select once; TCC remembers the grant forever, and every subsequent render is silent. Alternative: grant Word **Full Disk Access** in System Settings → Privacy & Security to skip all prompts. The prompt fires on every pytest run though (fresh `/private/var/folders/.../pytest-NN/` path each time).
  - **Portability seam:** `regenerate_fields()` is the only piece that needs rewriting for a future Linux deployment. Callers don't know or care which backend is used. macOS + Word today; `libreoffice --headless` with a Basic macro (or Aspose.Words) on Linux tomorrow.
